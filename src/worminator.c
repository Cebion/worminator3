...
else add_console_line("Skipping introduction.");
...
if (playing_demo) read_bytes((unsigned char*)input_buffer, sizeof(input_buffer), demofile);
else if (console_state == 1 || console_state == 2) for (i = 0; i < 28; i++) {
...
if (!playing_demo && !recording_demo) {
	...
	if (key[KEY_F2]) {
		if (wormy_dummy_box(3, FALSE) == TRUE) save_game(save_game_filename());
		return;
		}
...
if (input_buffer[INPUT_FIRE]) { fire_key_state++; override_autoswitch = FALSE; }
...
if (worminator_map.info_tile_grid[(int)((player.x_position_in_pixels + 8) / 16)][(int)((player.y_position_in_pixels + 8) / 16)] == water_info_tile || worminator_map.info_tile_grid[(int)((player.x_position_in_pixels + 24) / 16)][(int)((player.y_position_in_pixels + 8) / 16)] == water_info_tile) {
	...
	if (player.in_water == TRUE && player.y_acceleration < 0) { player.turbo_launch = TRUE;  player.y_acceleration = -1.7; }
...
else if (player.skin == TANK_SKIN && fire_key_state > 64) fire_key_state = 0;
else if (player.skin != TANK_SKIN && player.current_weapon == 1 && fire_key_state > 10) fire_key_state = 0;
else if (player.skin != TANK_SKIN && player.current_weapon == 2 && fire_key_state > 128) fire_key_state = 0;
else if (player.skin != TANK_SKIN && player.current_weapon == 3 && fire_key_state > 64) fire_key_state = 0;
else if (player.skin != TANK_SKIN && player.current_weapon == 4 && fire_key_state > 128) fire_key_state = 0;
else if (player.skin != TANK_SKIN && player.current_weapon == 5 && fire_key_state > 32) fire_key_state = 0;
else if (player.skin != TANK_SKIN && player.current_weapon == 6 && fire_key_state > 46) fire_key_state = 0;
else if (player.skin != TANK_SKIN && player.current_weapon == 7 && fire_key_state > 96) fire_key_state = 0;
else if (player.skin != TANK_SKIN && player.current_weapon == 8 && fire_key_state > 384) fire_key_state = 0;
else if (player.skin != TANK_SKIN && player.current_weapon == 9 && fire_key_state > 256) fire_key_state = 0;
else if (player.skin != TANK_SKIN && player.current_weapon == 10 && fire_key_state > 9) fire_key_state = 0;
else if (player.skin != TANK_SKIN && player.current_weapon == 11 && fire_key_state > 512) fire_key_state = 0;
else if (player.skin != TANK_SKIN && player.current_weapon == 12 && fire_key_state > 1024) fire_key_state = 0;
else if (player.skin != TANK_SKIN && player.current_weapon == 0) fire_key_state = 0;
...
if (input_buffer[INPUT_WEAPON_0]) { desired_weapon = 0; override_autoswitch = TRUE; }
if (input_buffer[INPUT_WEAPON_1]) { desired_weapon = 1; override_autoswitch = TRUE; }
if (input_buffer[INPUT_WEAPON_2]) { desired_weapon = 2; override_autoswitch = TRUE; }
if (input_buffer[INPUT_WEAPON_3]) { desired_weapon = 3; override_autoswitch = TRUE; }
if (input_buffer[INPUT_WEAPON_4]) { desired_weapon = 4; override_autoswitch = TRUE; }
if (input_buffer[INPUT_WEAPON_5]) { desired_weapon = 5; override_autoswitch = TRUE; }
if (input_buffer[INPUT_WEAPON_6]) { desired_weapon = 6; override_autoswitch = TRUE; }
if (input_buffer[INPUT_WEAPON_7]) { desired_weapon = 7; override_autoswitch = TRUE; }
if (input_buffer[INPUT_WEAPON_8]) { desired_weapon = 8; override_autoswitch = TRUE; }
if (input_buffer[INPUT_WEAPON_9]) { desired_weapon = 9; override_autoswitch = TRUE; }
if (input_buffer[INPUT_WEAPON_10]) { desired_weapon = 10; override_autoswitch = TRUE; }
if (input_buffer[INPUT_WEAPON_11]) { desired_weapon = 11; override_autoswitch = TRUE; }
if (input_buffer[INPUT_WEAPON_12]) { desired_weapon = 12; override_autoswitch = TRUE; }
...
if (player.no_control == TRUE) control_player();

// Water keyscan movement code
else if (player.in_water == TRUE) {
	if (input_buffer[INPUT_UP] || (input_buffer[INPUT_JUMP] && !input_buffer[INPUT_DOWN])) player.y_acceleration -= .02;
	else if (input_buffer[INPUT_DOWN]) player.y_acceleration += .02;
	else {
	...
	if (input_buffer[INPUT_LEFT]) player.x_acceleration -= .02, player.direction = 1;
	else if (input_buffer[INPUT_RIGHT]) player.x_acceleration += .02, player.direction = 0;
	else {
...
else if (!input_buffer[INPUT_DUCK] && player.is_ducking == TRUE) player.is_ducking = FALSE;
...
// Normal keyscan movement code
	...
	if (wormy_config.super_turkey_mode && jump_key_state == 1 && !input_buffer[INPUT_DOWN]) { if (player.y_acceleration > 0) player.y_acceleration = 0; player.y_acceleration -= 1.2; if (player.y_acceleration < -1.7) player.y_acceleration = -1.7; }
	}
...
if (player.no_control == TRUE) {}
	...
	if (player.y_acceleration != 0 && player.on_ground == FALSE) {
		if (player.y_acceleration > 0) {
			...
			if (player.y_acceleration <= 1) { if (player.no_control == FALSE) push_player(0, player.y_acceleration); }
			else {
...
for (i = (player.x_position_in_pixels / 16) + 1; i >= 0; i--) if (worminator_map.graphical_properties_tile_grid[i][j] == scroll_stop_grph_tile || i == 0) { topx = i; break; }
i = (player.x_position_in_pixels / 16) + 1; j = (player.y_position_in_pixels / 16) + 1;
for (j = (player.y_position_in_pixels / 16) + 1; j >= 0; j--) if (worminator_map.graphical_properties_tile_grid[i][j] == scroll_stop_grph_tile || j == 0) { topy = j; break; }
i = (player.x_position_in_pixels / 16) + 1; j = (player.y_position_in_pixels / 16) + 1;
for (i = (player.x_position_in_pixels / 16) + 1; i < worminator_map.map_width; i++) if (worminator_map.graphical_properties_tile_grid[i][j] == scroll_stop_grph_tile || i == worminator_map.map_width - 1) { bottomx = i + 1; break; }
i = (player.x_position_in_pixels / 16) + 1; j = (player.y_position_in_pixels / 16) + 1;
for (j = (player.y_position_in_pixels / 16) + 1; j < worminator_map.map_height; j++) if (worminator_map.graphical_properties_tile_grid[i][j] == scroll_stop_grph_tile || j == worminator_map.map_height - 1) { bottomy = j + 1; break; }
...
if (wormy_config.enable_FSAA == TRUE) {
	...
	else blit(FSAA_buffer, screen, 0, 0, (int)(screen_width * 0.0125), (int)(screen_height * 0.02), (int)(screen_width * .8), (int)(screen_height * .96));
...
for (sprite_loop = 0; sprite_loop < number_of_sprites; sprite_loop++) {
	if (sprites[sprite_loop].sprite_type == STARDUST_PARTICLE_SPRITE) {
	...
	else if (sprites[sprite_loop].sprite_type == CIGARETTE_BUTT_ENEMY_SPRITE) rotate_scaled_sprite(double_buffer, worminator_data_file[sprites[sprite_loop].frame].dat, (int)(sprites[sprite_loop].x_position_in_pixels) - screen_x_position, (int)(sprites[sprite_loop].y_position_in_pixels) - screen_y_position, itofix(sprites[sprite_loop].rotate_factor), ftofix(sprites[sprite_loop].timer / 128.0));
		...
		if (sprites[sprite_loop].sprite_type == MEDIUM_EXPLOSION_SPRITE || sprites[sprite_loop].sprite_type == SMALL_EXPLOSION_SPRITE) draw_trans_sprite(double_buffer, worminator_data_file[sprites[sprite_loop].frame].dat, (int)(sprites[sprite_loop].x_position_in_pixels - screen_x_position), (int)(sprites[sprite_loop].y_position_in_pixels - screen_y_position));
...
for(x_tile_loop = 0; x_tile_loop < ((int)(screen_width * .8) / 16) + 2; x_tile_loop++) {
	for(y_tile_loop = 0; y_tile_loop < ((int)(screen_height * .96) / 16) + 2; y_tile_loop++) {
		...
		if (worminator_map.foreground_tile_grid[start_x_tile + x_tile_loop][start_y_tile + y_tile_loop] != 0) {
			...
			else if (worminator_map.graphical_properties_tile_grid[start_x_tile + x_tile_loop][start_y_tile + y_tile_loop] != masked_foreground_grph_tile && worminator_map.graphical_properties_tile_grid[start_x_tile + x_tile_loop][start_y_tile + y_tile_loop] != masked_everything_grph_tile && worminator_map.graphical_properties_tile_grid[start_x_tile + x_tile_loop][start_y_tile + y_tile_loop] != hide_grph_tile) masked_blit(worminator_tiles, double_buffer, worminator_map.foreground_tile_grid[start_x_tile + x_tile_loop][start_y_tile + y_tile_loop] * 16, 0, x_tile_loop * 16 - sx, y_tile_loop * 16 - sy, 16, 16);
			}
		}
	}

// Draw a laser sight if nessicary
if (wormy_config.show_laser_sight == TRUE && player.current_weapon == 2 && player.shells_left >= 0 && player.skin != TANK_SKIN) draw_sprite(double_buffer, worminator_data_file[SHOTGUN_LASER_SIGHT].dat, find_laser_sight() - screen_x_position - 2, player.y_position_in_pixels + 15 - screen_y_position - 1);
...
if ((worminator_map.info_tile_grid[(int)((player.x_position_in_pixels - 4) / 16)][(int)(player.y_position_in_pixels / 16)] >= teleporter_1_info_tile && worminator_map.info_tile_grid[(int)((player.x_position_in_pixels - 4) / 16)][(int)(player.y_position_in_pixels / 16)] <= teleporter_16_info_tile) || (worminator_map.info_tile_grid[(int)((player.x_position_in_pixels + 7) / 16)][(int)(player.y_position_in_pixels / 16)] >= teleporter_1_info_tile && worminator_map.info_tile_grid[(int)((player.x_position_in_pixels + 7) / 16)][(int)(player.y_position_in_pixels / 16)] <= teleporter_16_info_tile) || (worminator_map.info_tile_grid[(int)((player.x_position_in_pixels + 18) / 16)][(int)(player.y_position_in_pixels / 16)] >= teleporter_1_info_tile && worminator_map.info_tile_grid[(int)((player.x_position_in_pixels + 18) / 16)][(int)(player.y_position_in_pixels / 16)] <= teleporter_16_info_tile)) {
	if (global_timer_3 < 32) draw_sprite(double_buffer, worminator_data_file[PINK_ARROW_SPIN_01 + (int)(global_timer_3 / 8)].dat, player.x_position_in_pixels - screen_x_position, player.y_position_in_pixels - screen_y_position - 40 + ((global_timer_3 / 2) - 8) / 2);
	else draw_sprite(double_buffer, worminator_data_file[PINK_ARROW_SPIN_01 + (int)(global_timer_3 / 8)].dat, player.x_position_in_pixels - screen_x_position, player.y_position_in_pixels - screen_y_position - 40 + (16 - ((global_timer_3 / 2) - 16) - 8) / 2);
	}
...
for (sprite_loop = 0; sprite_loop < number_of_sprites; sprite_loop++) {
	if (sprites[sprite_loop].sprite_type == PERVERT_MESSAGE_SPRITE || sprites[sprite_loop].sprite_type == BORED_SPRITE) draw_sprite(double_buffer, worminator_data_file[sprites[sprite_loop].frame].dat, (int)(sprites[sprite_loop].x_position_in_pixels) - screen_x_position, (int)(sprites[sprite_loop].y_position_in_pixels) - screen_y_position);
	}
...
if (show_fps == TRUE) textout(double_buffer, font, message, (int)(screen_width * .8) - 128, 0, 5);
...
if (playing_demo && mouse_frame >= 4) draw_sprite(double_buffer, worminator_data_file[DEMO_SIGN].dat, ((int)(screen_width * .8) / 2) - 64, ((int)(screen_height * .96) / 2) - 64);

// Draw wormcode if nessicary
if (bullet_time_engaged && wormy_config.cool_mode == TRUE) draw_wormcode();

// Draw the console if nessicary
if (console_state != 0) draw_console(double_buffer, (float)((float)(console_position) / 100.0) * (screen_height / 2));
...
if (wormy_config.enable_FSAA == FALSE) {
	...
	else blit(double_buffer, screen, 0, 0, (int)(screen_width * 0.0125), (int)(screen_height * 0.02), (int)(screen_width * .8), (int)(screen_height * .96));
	}
...
for (x = 0; x < 256; x++) {
	for (y = 0; y < 192; y++) {
		// Off map
		if (x > worminator_map.map_width || y > worminator_map.map_height) putpixel(double_buffer, x, y, makecol(128, 128, 128));
		
		// Solid tiles
		else if (worminator_map.info_tile_grid[x][y] == solid_info_tile) putpixel(double_buffer, x, y, makecol(255, 255, 255));
		else if (worminator_map.info_tile_grid[x][y] == conveyor_belt_right_info_tile || worminator_map.info_tile_grid[x][y] == conveyor_belt_left_info_tile) putpixel(double_buffer, x, y, makecol(255, 255, 255));
		else if (worminator_map.info_tile_grid[x][y] == half_slippery_info_tile || worminator_map.info_tile_grid[x][y] == mostly_slippery_info_tile) putpixel(double_buffer, x, y, makecol(255, 255, 255));
		...
		else if (worminator_map.info_tile_grid[x][y] >= solid_from_top_no_drop_info_tile && worminator_map.info_tile_grid[x][y] <= solid_from_left_no_ammo_info_tile) putpixel(double_buffer, x, y, makecol(128, 128, 128));
		else if (worminator_map.info_tile_grid[x][y] == slope_up_left_to_right_45_info_tile || worminator_map.info_tile_grid[x][y] == slope_up_right_to_left_45_info_tile) putpixel(double_buffer, x, y, makecol(128, 128, 128));
		else if (worminator_map.info_tile_grid[x][y] == slope_up_left_to_right_225_right_info_tile || worminator_map.info_tile_grid[x][y] == slope_up_right_to_left_225_left_info_tile) putpixel(double_buffer, x, y, makecol(128, 128, 128));
		else if (worminator_map.info_tile_grid[x][y] == breakable_wall_info_tile) putpixel(double_buffer, x, y, makecol(128, 128, 128));

		// Teleporters and exits
		else if (worminator_map.info_tile_grid[x][y] >= teleporter_1_info_tile && worminator_map.info_tile_grid[x][y] <= quick_teleporter_16_info_tile) putpixel(double_buffer, x, y, makecol(0, 255, 255));
		else if (worminator_map.info_tile_grid[x][y] >= quick_teleporter_exit_1_info_tile && worminator_map.info_tile_grid[x][y] <= quick_teleporter_exit_16_info_tile) putpixel(double_buffer, x, y, makecol(0, 255, 255));
		else if (worminator_map.info_tile_grid[x][y] >= exit_to_level_1_info_tile && worminator_map.info_tile_grid[x][y] <= quick_exit_to_level_16_info_tile) putpixel(double_buffer, x, y, makecol(255, 255, 0));
...
for (scanner = 0; scanner < number_of_sprites; scanner++) {
	if (sprites[scanner].sprite_active != 0) {
		if (sprites[scanner].sprite_type >= BLUE_SLIME_ENEMY_SPRITE && sprites[scanner].sprite_type <= BOSS_ENEMY_SPRITE && sprites[scanner].hit_points_left != -666) putpixel(double_buffer, (sprites[scanner].x_position_in_pixels / 16), (sprites[scanner].y_position_in_pixels / 16), makecol(255, 0, 0));
		else if (sprites[scanner].sprite_type >= STOMPING_FOOT_ENEMY_SPRITE && sprites[scanner].sprite_type <= CIGARETTE_BUTT_ENEMY_SPRITE) putpixel(double_buffer, (sprites[scanner].x_position_in_pixels / 16), (sprites[scanner].y_position_in_pixels / 16), makecol(255, 0, 0));
		else if (sprites[scanner].sprite_type >= HEALTH_1_SPRITE && sprites[scanner].sprite_type <= MUD_BEER_SPRITE) putpixel(double_buffer, (sprites[scanner].x_position_in_pixels / 16), (sprites[scanner].y_position_in_pixels / 16), makecol(0, 255, 0));
		else if (sprites[scanner].sprite_type >= PLASMA_BULLETS_SMALL_SPRITE && sprites[scanner].sprite_type <= BIG_TASTY_BALL_O_DIRT_SPRITE) putpixel(double_buffer, (sprites[scanner].x_position_in_pixels / 16), (sprites[scanner].y_position_in_pixels / 16), makecol(0, 255, 0));
		else if (sprites[scanner].sprite_type == FLOATING_PLATFORM_SPRITE) { putpixel(double_buffer, (sprites[scanner].x_position_in_pixels / 16), (sprites[scanner].y_position_in_pixels / 16), makecol(255, 255, 255));  putpixel(double_buffer, (sprites[scanner].x_position_in_pixels / 16) + 1, (sprites[scanner].y_position_in_pixels / 16), makecol(255, 255, 255)); }
		else if (sprites[scanner].sprite_type == RED_DOOR_SPRITE && sprites[scanner].action_state == 0) { putpixel(double_buffer, (sprites[scanner].x_position_in_pixels / 16), (sprites[scanner].y_position_in_pixels / 16), makecol(255, 0, 0));  putpixel(double_buffer, (sprites[scanner].x_position_in_pixels / 16), (sprites[scanner].y_position_in_pixels / 16) + 1, makecol(255, 0, 0)); }
		else if (sprites[scanner].sprite_type == GREEN_DOOR_SPRITE && sprites[scanner].action_state == 0) { putpixel(double_buffer, (sprites[scanner].x_position_in_pixels / 16), (sprites[scanner].y_position_in_pixels / 16), makecol(0, 255, 0));  putpixel(double_buffer, (sprites[scanner].x_position_in_pixels / 16), (sprites[scanner].y_position_in_pixels / 16) + 1, makecol(0, 255, 0)); }
		else if (sprites[scanner].sprite_type == BLUE_DOOR_SPRITE && sprites[scanner].action_state == 0) { putpixel(double_buffer, (sprites[scanner].x_position_in_pixels / 16), (sprites[scanner].y_position_in_pixels / 16), makecol(0, 0, 255));  putpixel(double_buffer, (sprites[scanner].x_position_in_pixels / 16), (sprites[scanner].y_position_in_pixels / 16) + 1, makecol(0, 0, 255)); }
		else if (sprites[scanner].sprite_type == SMALL_EXPLOSIVE_BARREL_SPRITE || sprites[scanner].sprite_type == SMALL_EXPLOSIVE_NUCLEAR_CRATE_SPRITE) { putpixel(double_buffer, (sprites[scanner].x_position_in_pixels / 16), (sprites[scanner].y_position_in_pixels / 16), makecol(255, 255, 0));  putpixel(double_buffer, (sprites[scanner].x_position_in_pixels / 16), (sprites[scanner].y_position_in_pixels / 16) + 1, makecol(255, 255, 0)); }
		}
	}
...
//////// Music ////////////////////////////////////////////////////////////////////////////////
	if (player_just_died == FALSE) {
		if (wormy_config.cool_mode == TRUE) play_midi(worminator_data_file[COOL_MODE_MUSIC].dat, TRUE);
		else {
...
wormy_config.music_volume = get_config_int("Options", "music_volume", 175);
...
remove_sound();
if (wormy_config.disable_sound == FALSE && wormy_config.disable_music == FALSE) install_sound(DIGI_AUTODETECT, MIDI_AUTODETECT, 0);
else if (wormy_config.disable_sound == FALSE && wormy_config.disable_music == TRUE) install_sound(DIGI_AUTODETECT, 0, 0);
else if (wormy_config.disable_sound == TRUE && wormy_config.disable_music == FALSE) install_sound(0, MIDI_AUTODETECT, 0);
